...
- (void)viewDidLoad
{
    ...
    [dataSource getEntityListOfIndustry:AXC];
    [dataSource release];
...
-(void)storeRefreshTime
{
    ...
    [defaults setDouble:[[NSDate date] timeIntervalSince1970] forKey:@"AXCTimeStamp"];
...
{
    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
    double prevTimestamp = [defaults doubleForKey:@"AXCTimeStamp"];
...
-(void)refreshData
{
    ...
    DataSource* dataSource = [[DataSource alloc] init];
    dataSource.delegate = self;
    [dataSource refreshDataOfIndustry:AXC];
    [dataSource release];
}
...
{
    AXCDetailsVC* detailsVC = [[AXCDetailsVC alloc] initWithEntityItem:[dataSourceArray objectAtIndex:indexPath.row]];
