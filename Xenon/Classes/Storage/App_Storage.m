...
NSString* const AXC_ENTITYSUBITEMSLIST  =@"create table if not exists AXC_EntitySubItemsList (_id integer auto increment primary key, entitySubitemTitle text null,entitySubitemValue text null,foreignId text null);"; 
...
NSString* const AXC_ENTITY_SUBITEMS = @"AXC_EntitySubItemsList";
...
-(BOOL)initVars
{
    ...
    if (sqlite3_open([documentsDir UTF8String], &database) == SQLITE_OK) 
    {
        ...
        [self checkAndCreateDatabase:AXC_ENTITYSUBITEMSLIST];
...
-(void)deleteEntityListFromTable:(NSString*)table
{
    NSString* sqlQuery = [[NSString alloc] initWithFormat:@"%@%@",@"delete from ",table];
    sqlite3_stmt* statement = NULL;
...
-(void)storeEntitySubItemsIntoTable:(NSString*)table ContentValues:(NSDictionary*)contentValues
{
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    
    NSString* sqlQuery = [[NSString alloc] initWithFormat:@"%@%@%@",@"insert into ",table,@"(entitySubitemTitle,entitySubitemValue,foreignId) values(?,?,?)"];
    NSLog(@"query is %@",sqlQuery);
...
-(NSMutableArray*)getEntityListSubItemsFromTable:(NSString*)table ForEntity:(NSString*)entity
{
    NSString* sqlQuery = [[NSString alloc] initWithFormat:@"%@%@%@%@%@",@"select * from ",table,@" where foreignId = '",entity,@"'"];
